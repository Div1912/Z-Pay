...
const CARD_DETAILS: Record<string, { description: string; tips: string[] }> = {
  "Total Users":    { description: "Unique accounts that have claimed a @Zp Universal ID and activated a Stellar wallet.", tips: ["Share your referral link to grow faster", "Each user gets a Stellar ${process.env.NEXT_PUBLIC_STELLAR_NETWORK === 'mainnet' ? 'mainnet' : 'testnet'} wallet on signup", "Target: 30+ for Black Belt"] },
...
export default function MetricsDashboard() {
  ...
  const kpis = summary ? [
    ...
    { key: "Volume (30d)",   value: summary.volume_30d,            icon: BarChart2,  color: "#facc15", pct: 100, sparkData: daily.slice(-14).map(d => d.volume), unit: "XLM", sub: `Stellar ${process.env.NEXT_PUBLIC_STELLAR_NETWORK === 'mainnet' ? 'mainnet' : 'testnet'}` },
