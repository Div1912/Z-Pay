        ...
        {receiver && quote && (
          <div className="p-4 bg-white/5 rounded-2xl space-y-3">
            <div className="flex items-center gap-3">
              <div
                className="w-10 h-10 bg-gold/20 rounded-xl flex items-center justify-center font-black text-gold bg-cover bg-center overflow-hidden"
          ...
          {[0, 1, 2, 3].map((index) => (
            <input
              ...
              className={cn(
                ...
                pinError ? "border-red-500/50 focus:border-red-500" : "border-white/10 focus:border-gold"
              )}
              disabled={loading}
            />
          ))}
...
function SendForm() {
  ...
  if (status === "success" && paymentResult) {
    return (
      ...
      >
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-gold to-green-500" />
        <motion.div 
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
        ...
        <div className="max-w-sm mx-auto space-y-4 mb-8">
          ...
          {paymentResult.from_currency !== paymentResult.to_currency && (
            <div className="p-4 bg-gold/10 rounded-2xl border border-gold/20">
              ...
              <p className="text-3xl font-black text-gold">~{paymentResult.amount_received?.toFixed(2)} {paymentResult.to_currency}</p>
            </div>
          )}
        </div>
        <div className="space-y-3">
          <a 
            ...
            className="flex items-center justify-center gap-2 text-zinc-500 hover:text-gold transition-all font-bold text-sm bg-white/5 py-4 rounded-xl border border-white/5"
          >
          ...
          <Button
            ...
            className="w-full h-14 bg-gold/15 hover:bg-gold/25 border border-gold/30 text-gold font-black text-base rounded-2xl transition-all"
          >
            PAY AGAIN
          </Button>
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              <Input
                ...
                className="bg-white/5 border-white/10 pl-14 h-16 text-xl font-bold rounded-2xl focus:border-gold/50"
                value={recipient}
          ...
          <AnimatePresence>
            {receiverProfile && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="p-5 bg-gold/10 rounded-2xl border border-gold/20 space-y-4">
                  <div className="flex items-center gap-4">
                    <div
                      className="w-14 h-14 bg-gold/20 rounded-2xl flex items-center justify-center text-2xl font-black text-gold bg-cover bg-center overflow-hidden"
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-black text-gold">{receiverProfile.preferred_currency}</p>
                    </div>
                  </div>
                  <div className={cn(
...
<div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-gold px-2 py-0.5 bg-gold/10 rounded-md">
              ...
              <div className="relative group">
                <Input
                  ...
                  step="0.01"
                  placeholder="0.00"
                  className="bg-white/5 border-white/10 h-20 text-4xl font-black pr-24 rounded-2xl focus:border-gold/50"
                  value={amount}
            ...
            <div className="space-y-3">
              ...
              <select
                ...
                className="w-full bg-white/5 border border-white/10 h-14 px-5 text-base font-bold rounded-2xl focus:border-gold/50 focus:outline-none appearance-none cursor-pointer"
              >
            ...
            {(receiverProfile?.address.startsWith('G') && receiverProfile?.username === "External Stellar Wallet") && (
              <div className="space-y-3">
                ...
                <Input
                  ...
                  className="bg-white/5 border-white/10 h-14 text-base font-bold rounded-2xl focus:border-gold/50"
                  value={memo}
              ...
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-5 bg-gradient-to-br from-white/5 to-white/[0.02] rounded-2xl border border-white/10 space-y-4">
                ...
                <div className="flex items-center justify-between py-4 border-y border-white/5">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-black text-gold">~{quote.target_amount.toFixed(2)} {quote.to_currency}</p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-white/5">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-gold">{useGasless ? "0.0000 XLM (Paid by Z-Pay)" : "0.00001 XLM (Standard)"}</span>
                    </div>
                    <div className="flex justify-between">
          ...
          <div 
            className={cn("flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all", 
              useGasless ? "bg-gold/10 border-gold/30" : "bg-white/5 border-white/10 hover:bg-white/10")}
            onClick={() => setUseGasless(!useGasless)}
          >
            <div className="flex items-center gap-3">
              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-colors", 
                useGasless ? "bg-gold/20" : "bg-white/10")}>
                <Zap className={cn("w-5 h-5 transition-colors", useGasless ? "text-gold" : "text-zinc-500")} />
              </div>
              <div>
                <p className={cn("font-bold transition-colors", useGasless ? "text-gold" : "text-white")}>Gasless Transaction ⚡</p>
                <p className="text-xs text-zinc-500">Platform pays the Stellar network fee</p>
              </div>
            </div>
            <div className={cn("w-12 h-6 rounded-full p-1 transition-colors", useGasless ? "bg-gold" : "bg-zinc-700")}>
          ...
          <Button 
            ...
            className="w-full h-20 bg-gold hover:bg-gold/90 text-black text-2xl font-black rounded-3xl shadow-2xl shadow-gold/30 disabled:opacity-50 group"
          >
            {loading ? (
