    ...
    <div className="max-w-lg mx-auto pb-24 space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-black text-gold">{step}<span className="text-white/20 text-sm">/3</span></p>
        </div>
      </div>
          ...
          <div key={s} className="flex-1 relative h-1 bg-white/[0.06] rounded-full overflow-hidden">
            {s <= step && (
              <motion.div
                ...
                className="absolute inset-0 bg-gradient-to-r from-gold to-[#27272a] rounded-full"
              />
            )}
          </div>
        ))}
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          <motion.div key="s1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-6">
            <div className="relative overflow-hidden rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6 space-y-5">
              ...
              <div className="space-y-2 relative z-10">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.04] border border-white/10 rounded-2xl px-5 py-4 font-bold text-base focus:outline-none focus:border-gold/40 focus:bg-white/[0.07] placeholder:text-white/15 transition-all"
                />
              </div>
              ...
              <div className="space-y-2 relative z-10">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full bg-white/[0.04] border border-white/10 rounded-2xl px-5 py-4 font-black text-4xl focus:outline-none focus:border-gold/40 focus:bg-white/[0.07] placeholder:text-white/10 transition-all pr-24"
                  />
              ...
              <div className="space-y-2 relative z-10">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.04] border border-white/10 rounded-2xl px-5 py-3 font-medium text-sm focus:outline-none focus:border-gold/40 placeholder:text-white/15 transition-all"
                />
              </div>
            </div>
            ...
            <motion.button
              ...
              onClick={handleStep1}
              className="w-full h-16 bg-gradient-to-r from-gold to-[#27272a] rounded-2xl font-black uppercase tracking-wider text-base shadow-2xl shadow-gold/25 flex items-center justify-center gap-2"
            >
          ...
          <motion.div key="s2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-6">
            ...
            <div className="relative flex p-1 bg-white/[0.03] border border-white/[0.06] rounded-2xl">
              {(["equal", "custom"] as const).map(type => (
                <button key={type} onClick={() => setSplitType(type)}
                  ...
                  {splitType === type && (
                    <motion.div layoutId="split-type" className="absolute inset-0 bg-gradient-to-r from-gold/20 to-[#27272a]/10 rounded-xl border border-gold/30"
                      transition={{ type: "spring", bounce: 0.2, duration: 0.4 }} />
                  )}
                  <span className={cn("relative z-10 flex items-center gap-2 transition-colors", splitType === type ? "text-gold" : "text-white/30")}>
            ...
            {splitType === "equal" && totalAmount && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                className="bg-gold/5 border border-gold/15 rounded-2xl px-5 py-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-white/30">Each person pays (incl. you)</p>
                    <p className="font-black text-2xl text-gold mt-0.5">{perPerson} <span className="text-sm text-white/30">XLM</span></p>
                  </div>
                ...
                <div className="flex gap-3 text-[10px] font-black uppercase tracking-wider">
                  ...
                  <span className="text-gold/70">You collect {(participants.length > 0 ? (parseFloat(totalAmount) - parseFloat(totalAmount) / (participants.length + 1)) : 0).toFixed(2)} XLM from friends</span>
                </div>
              </motion.div>
            )}
              ...
              {participants.map((p, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className={cn(
                    ...
                    p.resolved ? "border-gold/30 bg-gold/5" : "border-white/[0.08] focus-within:border-gold/30"
                  )}>
                    {resolving === i ? (
                      <Loader2 className="w-8 h-8 text-gold animate-spin shrink-0" />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-gold/60 font-black text-sm">@</span>
                        <input
                  ...
                  {splitType === "custom" && (
                    <div className="w-28 bg-white/[0.03] border border-white/[0.08] rounded-2xl px-3 py-3 flex items-center gap-1 focus-within:border-gold/30 transition-all">
                      <input
                        type="number"
                        value={p.amount_owed}
        ...
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-6">
            ...
            <div className="relative overflow-hidden rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6 space-y-5">
              <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-gold to-[#27272a] opacity-60" />
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <p className="font-black text-2xl text-gold">{totalAmount}</p>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
              ...
              className="w-full h-16 bg-gradient-to-r from-gold to-[#27272a] rounded-2xl font-black uppercase tracking-wider text-base shadow-2xl shadow-gold/25 disabled:opacity-50 flex items-center justify-center gap-3">
