...
export default function SplitDetailPage() {
  ...
  if (!data) return (
    <div className="text-center py-20">
      ...
      <Link href="/dashboard/split" className="text-gold text-sm font-black mt-4 inline-block">← Back to Splits</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-lg mx-auto space-y-8 pb-20">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">
            ...
            {" · "}
            <span className={cn(
              split.status === 'completed' ? "text-green-400" : "text-gold"
            )}>
              {split.status}
            </span>
          </p>
      ...
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-3xl overflow-hidden">
        {/* Amount header */}
        <div className="p-6 bg-gradient-to-br from-gold/5 to-[#27272a]/5 border-b border-white/5">
          ...
          <div className="mt-4 space-y-2">
            <div className="flex justify-between text-xs font-bold text-white/40">
              ...
              <span className="text-gold">{collectedSoFar.toFixed(2)} / {split.total_amount} {split.currency}</span>
            </div>
          ...
          >
            <div className="bg-gold/5 border border-gold/20 rounded-2xl px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="font-black text-2xl text-gold mt-0.5">
            ...
            <button
              ...
              onClick={handlePay}
              disabled={paying}
              className="w-full h-16 bg-gradient-to-r from-gold to-[#27272a] rounded-2xl font-black uppercase tracking-wider text-lg shadow-2xl shadow-gold/30 hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
            >
              {paying ? (
