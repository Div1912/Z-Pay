...
An arbiter will review the evidence and decide. Contact support@expopay.app with your dispute reason.`
  ...
  try {
    if (isPayer) {
      ...
      const txHash = await refundEscrow(String(contract.escrow_id), callerProfile.stellar_secret);
      ...
      const txHash        = await releaseEscrow(String(contract.escrow_id), payerProfile.stellar_secret);
