...
Your Universal ID is your permanent @Zp address — like `alice@Zp`.

1. After signup, you'll be taken to the **Onboarding** page
2. Choose your **@Zp username** (lowercase, no spaces, e.g. `alice`)
...
2. In the **To** field, type a `@Zp` username (e.g. `bob@Zp`) or start typing — suggestions appear
...
3. The confirmation will show: *"Transaction fee sponsored by Zpay"*
...
3. Or share your **@Zp ID** verbally: just say `alice@Zp`
...
Split any expense among `@Zp` users.
...
4. Add participants by typing their `@Zp` username
...
Lock your ZPAY tokens to earn fixed rewards:
...
2. Under **ZPAY Staking**, enter an amount and select a lock period
...
Deposit XLM with **no lock-up** and earn ZPAY rewards at ~18% APR.

1. Go to **Vault** → **Yield Pool** tab
2. Enter the XLM amount and click **Deposit**
3. Rewards accrue at 0.5% per XLM per day
4. Click **Withdraw** anytime to receive your XLM + accrued ZPAY
...
2. Enter the freelancer's `@Zp` ID, amount, title, and description
3. Set an **expiry date** (e.g., 30 days)
4. Click **Create** — this registers the contract on-chain (Soroban)
5. Click **Fund** and enter your PIN — ZPAY tokens are locked in the contract
...
Disputes are reviewed by the Zpay arbiter:
...
- **Never share your PIN** — Zpay will never ask for your PIN via email or chat
...
A: Double-check the @Zp username — it's case-sensitive. The `@Zp` suffix is added automatically.
...
- **GitHub**: [github.com/Div1912/Zpay](https://github.com/Div1912/Zpay/issues)
