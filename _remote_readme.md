...
# Zpay ΓÇö Global Payment Router

**Cross-border payments, escrow, group bills, and on-chain savings ΓÇö all on Stellar.**

Zpay turns wallet addresses into human-readable Universal IDs (`alice@Zp`), settles payments in seconds via Stellar, lets Indian merchants receive INR via UPI, lets freelancers and clients lock milestone funds in a Soroban contract, splits group bills automatically, and provides an on-chain Vault for ZPAY staking and daily XLM yield.

[**Live demo ΓåÆ**](https://exporouter.site) &nbsp;┬╖&nbsp; ![CI](https://github.com/Div1912/Zpay/actions/workflows/ci.yml/badge.svg)

[**MVP Video ΓåÆ**](https://youtu.be/nHyg3oPxe0Q)

[**User feedbacks ΓåÆ**](https://docs.google.com/spreadsheets/d/e/2PACX-1vR82azl8byhjpi6hAnn8naPIsU5H-I_TGDyDFqdP2jv7xJXpp5O1MSdHBfHmFYH0v7Bka2FSSyrEbS2/pubhtml)
...
- **Universal IDs** ΓÇö send to `alice@Zp` instead of a 56-char Stellar public key.
- **Instant P2P** ΓÇö XLM/USDC/INR P2P transfers settled on Stellar in ~3 seconds.
- **Escrow with arbiter** ΓÇö Soroban contract handles fund/deliver/release/dispute/resolve. Two-step arbiter override pays the freelancer even when the on-chain `release` is locked.
- **Split bills** ΓÇö equal or custom-share bill splitting across `@Zp` users, with per-participant payment tracking and notifications.
- **On-chain Vault** ΓÇö fixed-term ZPAY staking (30/60/90 days, up to 6%) plus a no-lock XLM yield pool that mints ZPAY rewards daily. Live accrual UI, compound projection, and a real-time earnings ticker.
- **Indian UPI bridge** ΓÇö pay any UPI QR with crypto; merchant receives INR.
- **Cross-currency FX** ΓÇö XLM Γåö USDC Γåö INR/USD/EUR/GBP with locked-window quotes.
- **Inactivity guard, transaction PINs, on-chain audit trail** ΓÇö every action emits a Stellar tx hash you can verify on Stellar Expert.
...
The home of the app ΓÇö wallet balance, recent transactions, quick actions for Send/Scan/Split/Vault.
...
Send to `bob@Zp` instead of `GAB6FΓÇª`. Cross-currency sends show a live FX quote with a locked window.
...
Every payment, escrow action, split contribution, and vault event in one place ΓÇö each row links to Stellar Expert for on-chain verification.
...
Create a bill, pick `@Zp` participants, choose **Equal** or **Custom shares**, and the app tracks who's paid and who hasn't. Each participant pays from their own balance with a single tap; the creator gets paid out directly.
...
- Each pay-in is an on-chain Stellar payment from participant ΓåÆ creator
- Status transitions: `active` ΓåÆ `partial` ΓåÆ `paid` (auto when all participants settle)

### 7 ┬╖ Vault ΓÇö staking + yield pool *(new)*

Two products in one tab. Live earnings ticker, animated stake progress, and a built-in compound-interest projection.

#### Staking

Lock ZPAY for 30, 60, or 90 days for **1.25% / 3.00% / 6.00%** flat reward (Γëê15 / 18 / 24% APR). Each active stake card shows current value, time remaining, accrued reward (animated 1 Hz), and unlocks at maturity.
...
Drag the slider for amount, tap a tier ΓÇö see what auto-rolling that tier yields vs simple interest over a year, with the true APY computed via discrete compounding `P ├ù (1+r)^n ΓêÆ P`.
...
### Escrow ΓÇö `contracts/escrow/src/lib.rs`

| Function | Description | Inter-contract call |
|---|---|---|
| `create` | Create an escrow with ZPAY `token_id` | ΓÇö |
| `fund` | Client locks ZPAY tokens in escrow | Γ£à Client ΓåÆ Escrow |
| `deliver` | Freelancer marks work as delivered | ΓÇö |
| `release` | Client releases ZPAY to freelancer | Γ£à Escrow ΓåÆ Freelancer |
| `refund` | Cancel and refund ZPAY to client | Γ£à Escrow ΓåÆ Client |
| `dispute` | Either party raises a dispute | ΓÇö |
| `resolve` | Arbiter distributes ZPAY to winner *(superseded ΓÇö see note below)* | Γ£à Escrow ΓåÆ Winner |
| `get` | Query escrow state | ΓÇö |

**Arbiter resolution note.** The current testnet build is a pre-`resolve` revision. Arbiter outcomes are handled in the API by:
- *Refund client* ΓåÆ `refund(escrow_id)` signed by the payer.
- *Pay freelancer* ΓåÆ `refund(escrow_id)` then a SEP-41 `transfer` from the payer's wallet to the freelancer's, both signed custodially. Net result equals a single resolve-to-freelancer call.

### Staking ΓÇö `contracts/staking/src/lib.rs`
...
Reward math: linear, flat-rate `reward = amount ├ù bps / 10000` over the lock duration. Stake states: `active ΓåÆ claimed`. Reward bps by tier: 30d ΓåÆ 125, 60d ΓåÆ 300, 90d ΓåÆ 600.

### XLM Yield Pool ΓÇö `contracts/pool/src/lib.rs`
...
Reward math: linear time-based accrual `accrued_expo = xlm_amount ├ù BASE_REWARD_BPS_PER_DAY ├ù elapsed_days / 10000` with `BASE_REWARD_BPS_PER_DAY = 50` (Γëê18% APR).
...
| GET | `/api/zpay/balance` | Wallet balances (XLM, ZPAY, ΓÇª) |
| GET | `/api/zpay/resolve?username=ΓÇª` | Resolve `@Zp` ID to a Stellar address |
...
| Method | Endpoint | Description |
|---|---|---|
| POST | `/api/merchant/quote` | Get XLMΓåöINR quote |
...
- `supabase_migration.sql` ΓÇö core tables (profiles, transactions, contracts, merchant_payments)
- `supabase_split_migration.sql` ΓÇö `split_bills`, `split_participants`
- `supabase_savings_migration.sql` ΓÇö `staking_positions`, `pool_positions`
...
SUPABASE_SERVICE_ROLE_KEY=...           # server-only ΓÇö REQUIRED in production
...
> ΓÜá∩╕Å The server now refuses to start in `production` if `SUPABASE_SERVICE_ROLE_KEY` is missing ΓÇö a deliberate guardrail so admin writes can't silently fall back to anon.
...
stellar contract deploy \
  ...
  --network testnet
# ΓåÆ returns the contract ID; paste into ESCROW_CONTRACT_ID
...
Zpay/
Γö£ΓöÇΓöÇ contracts/
Γöé   Γö£ΓöÇΓöÇ escrow/      # Soroban escrow contract
Γöé   Γö£ΓöÇΓöÇ staking/     # Fixed-term ZPAY staking
Γöé   ΓööΓöÇΓöÇ pool/        # XLM deposit pool with ZPAY rewards
Γö£ΓöÇΓöÇ scripts/
Γöé   Γö£ΓöÇΓöÇ deploy.ts        # Bulk-deploy all contracts
Γöé   ΓööΓöÇΓöÇ fund-rewards.ts  # Top up reward pools
Γö£ΓöÇΓöÇ src/
Γöé   Γö£ΓöÇΓöÇ app/
Γöé   Γöé   Γö£ΓöÇΓöÇ api/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ admin/{contracts,resolve}/   # Arbiter actions
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ contracts/{deliver,dispute,fund,refund,release}/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ zpay/{balance,check,check-phone,claim,pin,profile,resolve}/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ fx/quote/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ merchant/{history,pay,quote}/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ payments/{history,send}/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ savings/{positions,stake,unstake,pool/{deposit,withdraw}}/
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ split/[id]/pay/
Γöé   Γöé   Γö£ΓöÇΓöÇ auth/                     # Login, signup, OTP, reset
Γöé   Γöé   Γö£ΓöÇΓöÇ dashboard/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ admin/                # Arbiter console
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ contracts/            # Escrow UI
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ history/              # Tx history
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ merchant/             # Pay UPI
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ savings/              # Vault: staking + pool
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ scan/                 # QR scanner
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ send/                 # Send P2P
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ split/                # Split bills
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ receive/              # Show your QR
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ profile/  settings/
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ page.tsx              # Overview
Γöé   Γöé   Γö£ΓöÇΓöÇ onboarding/
Γöé   Γöé   ΓööΓöÇΓöÇ page.tsx                  # Landing
Γöé   Γö£ΓöÇΓöÇ components/
Γöé   Γöé   Γö£ΓöÇΓöÇ InactivityGuard.tsx       # Auto-logout w/ visibility-aware timers
Γöé   Γöé   Γö£ΓöÇΓöÇ Background.tsx  Logo.tsx  Navbar.tsx
Γöé   Γöé   Γö£ΓöÇΓöÇ PaymentNotification.tsx   # Realtime in-app alerts
Γöé   Γöé   Γö£ΓöÇΓöÇ sections/                 # Landing-page blocks
Γöé   Γöé   ΓööΓöÇΓöÇ ui/                       # Reusable primitives (Radix-based)
Γöé   Γö£ΓöÇΓöÇ lib/
Γöé   Γöé   Γö£ΓöÇΓöÇ stellar.ts                # Stellar SDK wrapper
Γöé   Γöé   Γö£ΓöÇΓöÇ escrow.ts                 # Escrow + token transfer helpers
Γöé   Γöé   Γö£ΓöÇΓöÇ savings.ts                # Staking & pool client
Γöé   Γöé   Γö£ΓöÇΓöÇ fx-service.ts             # Live FX quotes
Γöé   Γöé   Γö£ΓöÇΓöÇ upi-service.ts            # UPI QR parsing
Γöé   Γöé   Γö£ΓöÇΓöÇ notify.ts                 # Resend email helpers
Γöé   Γöé   Γö£ΓöÇΓöÇ supabase.ts               # Browser + admin clients
Γöé   Γöé   ΓööΓöÇΓöÇ supabase-server.ts        # Server-side getUser()
Γöé   ΓööΓöÇΓöÇ middleware.ts                 # Auth gate for /dashboard/* and /auth/*
Γö£ΓöÇΓöÇ supabase_migration.sql
Γö£ΓöÇΓöÇ supabase_split_migration.sql      # NEW
Γö£ΓöÇΓöÇ supabase_savings_migration.sql    # NEW
ΓööΓöÇΓöÇ screenshots/                      # README assets
```
...
- [ ] Stake streaks (consecutive completions ΓåÆ reward multiplier)
